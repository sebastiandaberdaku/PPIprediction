...
void MolecularSurface::extractPatchCenters(float minDistance, array3D const & interface, vector<voxel> & patchCenters) {
	assert(surface != NULL);
	/* discretized minimum distance between two different surface patches */
	float minD = minDistance * resolution;
	float s_minD = minD * minD;
...
void MolecularSurface::calculateSurfaceDescriptors(float patchRadius, int maxOrder,
		array3D const & interface, double threshold, std::vector<CompactPatchDescriptor> & descriptors) {
...
#pragma omp parallel for
	for (size_t i = 0; i < n; ++i) {
		SurfacePatch currentPatch(i, patchRadius, patchCenters[i], resolution,
				...
				interface, threshold, BLAM930101_, BIOV880101_, MAXF760101_,

...
bool CommandLineParser::parseCommandLine(int const & argc,
		char const * const argv[], float & patchRadius, float & minCenterDist,
	...
	//------------------------------- command line options --------------------------------------
        options_description description("PPI prediction - Testing samples generator. Usage");
...
Hydrogens are always present in PDB files resulting from NMR analysis, and are usually present in theoretical models.")
	 ...
	 */
	("patch_dist,d", value<patch_rd>()->default_value(1.8), "Minimum distance between patch centers (in Å), positive floating point (default is 1.8Å).")
	/*
	...
	try {
		...
		minCenterDist = vm["patch_dist"].as<patch_rd>().p;

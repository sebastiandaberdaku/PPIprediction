...
public:
	...
	vector<voxel> interface_patch_centers, other_patch_centers;

	Molecule(float patchRadius, float minCenterDist,
			float probeRadius, float resolution, string const & inname,
			string const & openDX, string const & outname, string const & inname_radii,
			int maxOrder, bool no_hydrogen, bool no_hetatm);
	virtual ~Molecule();
	...
	void calculateDescriptors(string const & outname, float minCenterDist_noninterface, float minCenterDist_interface, float patchRadius, int maxOrder, array3D const & interface, double threshold, vector<CompactPatchDescriptor> & out_descriptors) {
		auto t_start = chrono::high_resolution_clock::now();
		cout << "Extracting surface patches\n";
		surface->extractPatchCenters(minCenterDist_noninterface, other_patch_centers);
		cout << "Calculating Surface Descriptors\n";
		vector<CompactPatchDescriptor> int_d, nint_d;
		surface->calculateSurfaceDescriptors(patchRadius, maxOrder, other_patch_centers, interface, threshold, nint_d);
		size_t ip = 0, nip = 0;
		for (auto const & p : nint_d) {
			if (p.isInterface) {
				++ip;
			}
			else {
				++nip;
				out_descriptors.push_back(p);
			}
		}
		double ratio =  ip / (double) nip;
		cout << "Extracting interface patches\n";
		surface->extractInterfacePatchCenters(minCenterDist_interface, interface, interface_patch_centers);
		cout << "Calculating interface descriptors\n";
		surface->calculateSurfaceDescriptors(patchRadius, maxOrder, interface_patch_centers, interface, threshold, int_d);
		for (auto const & p : int_d) {
			if (p.isInterface)
				out_descriptors.push_back(p);
		}
...
//		auto t_start = chrono::high_resolution_clock::now();
//		cout << "Extracting interface patches\n";
//		surface->extractInterfacePatchCenters(minCenterDist, interface, interface_patch_centers);
//		cout << "Calculating interface descriptors\n";
//		surface->calculateSurfaceDescriptors(patchRadius, maxOrder, interface_patch_centers, interface, threshold, out_descriptors);
//		auto t_ms = chrono::duration_cast<chrono::milliseconds>(chrono::high_resolution_clock::now() - t_start).count();
...
////				out_descriptors.end());
//		interface_descriptors_count = out_descriptors.size();
//		cout << "Patch extraction and descriptor \ncalculation time:\t" << t_ms / 1000.0 << " seconds.\n"<<endl;
//	}

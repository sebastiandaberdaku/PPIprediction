...
Molecule::Molecule(float patchRadius, float minCenterDist,
		...
		int maxOrder, bool no_hydrogen, bool no_hetatm) :
			ASA(0), dG(0), SEV(0), pqrModel(NULL), pdbModel(NULL) {
	...
	cout << "Process " << omp_get_thread_num() << ": " << "Initializing parameters for " << inname <<".\n";
	cout << "Process " << omp_get_thread_num() << ": " << "Number of atoms in model: " << atoms->size() << "\n";
	MolecularSurface::boundingBox(*atoms, probeRadius, patchRadius, resolution, length,
			width, height, translation, max_atm_radius, min_atm_radius);
    auto t_start = chrono::high_resolution_clock::now();

	surface = new MolecularSurface(*atoms, probeRadius, resolution, length, width, height, translation);
	cout << "Process " << omp_get_thread_num() << ": " << "Creating the space-filling model\n";
	surface->createCPKModel();
	cout << "Process " << omp_get_thread_num() << ": " << "Filling internal cavities\n";
	surface->fillInternalCavities();
	cout << "Process " << omp_get_thread_num() << ": " << "Building SA surface\n";
...
//	outer_atoms_tree = kdtree_atom(&outer_atoms);
	...
	cout << "Process " << omp_get_thread_num() << ": " << "Calculating the Euclidean Distance Transform using \n";
	cout << "Process " << omp_get_thread_num() << ": " << "speed-optimized data structures\n";
...
//	surface->calculateAtomHydrophobicity(hphob_kapcha);
	...
	cout << "Process " << omp_get_thread_num() << ": " << "Calculating HQI8 indices.\n";
	surface->calculateHQI8();

	auto t_end = chrono::high_resolution_clock::now();
	int t_ms = chrono::duration_cast<chrono::milliseconds>(t_end - t_start).count();
	cout << "Process " << omp_get_thread_num() << ": " << "Surface calculation time:\t" << t_ms / 1000.0 << " seconds.\n";

...
bool CommandLineParser::parseCommandLine(int const & argc,
		char const * const argv[], float & patchRadius, float & minCenterDist_noninterface, float & minCenterDist_interface,
		...
		bool & no_hetatm, int & maxOrder, float & interface_distance, float & threshold,
		bool & help, bool & version, bool & surf_description, bool & license) {

	//------------------------------- command line options --------------------------------------
	options_description description("PPI prediction - Training samples generator. Usage");
	 ...
	 */
	("receptor", value<input_PDB_PQR_filename>()->required(), "Name of the receptor's PDB file.")
	/*
	 ...
	 * no option name are called "positional options" in the boost::program_options library.
	 */
	("ligand", value<input_PDB_PQR_filename>()->required(), "Name of the ligand's PDB file.")
	/*
...
Hydrogens are always present in PDB files resulting from NMR analysis, and are usually present in theoretical models.")
	 ...
	 */
	("patch_radius,R", value<patch_rd>()->default_value(6), "Patch radius (in Å), positive floating point (default is 6.0Å).")
	/*
	 * The -i(--interface_distance) flag is used to specify the threshold for the interface computation.
	 * Default is 4.5. Optional.
	 */
	("patch_threshold,t", value<patch_threshold>()->default_value(0.8), "Threshold value used for the interface patch determination (between 0 and 1, default is 0.8).")
	/*
	 * The -i(--interface_distance) flag is used to specify the threshold for the interface computation.
	 * Default is 4.5. Optional.
	 */
	("interface_distance,i", value<patch_rd>()->default_value(4.5), "Threshold distance for the interface determination (in Å), positive floating point (default is 4.5Å).")
	/*
	 * The -d (--patch_dist) flag is used to specify the minimum distance between patch centers.
	 * Default is 1.0. Optional.
	 */
	("nint_patch_dist,d", value<patch_rd>()->default_value(4.5), "Minimum distance between non-interface patch centers (in Å), positive floating point (default is 4.5Å).")
	("int_patch_dist,D", value<patch_rd>()->default_value(1.0), "Minimum distance between interface patch centers (in Å), positive floating point (default is 1.0Å).")
	...
	try {
		...
		if (vm.count("surf_description")) {
			cout << PROGRAM_NAME << "The program calculates the following surfaces:\n" << VWS << SAS << MS;
		...
		minCenterDist_noninterface = vm["nint_patch_dist"].as<patch_rd>().p;
		minCenterDist_interface = vm["int_patch_dist"].as<patch_rd>().p;

		interface_distance = vm["interface_distance"].as<patch_rd>().p;
		threshold = vm["patch_threshold"].as<patch_threshold>().t;

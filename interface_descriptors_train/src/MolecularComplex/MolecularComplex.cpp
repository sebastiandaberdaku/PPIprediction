...
MolecularComplex::MolecularComplex(Molecule const & receptor, Molecule const & ligand, float interface_distance /** 4.5Å distance used to determine if a surface voxel belongs to the interface*/) :
...
//	r_negativeDXPotentials = array3D(receptor.length, array2D(receptor.width, array1D(receptor.height, 0)));
//	r_positiveDXPotentials = array3D(receptor.length, array2D(receptor.width, array1D(receptor.height, 0)));
		...
		for (int j = 0; j < receptor.width; ++j) {
			for (int k = 0; k < receptor.height; ++k) {
				if (receptor.surface->surface->getVoxel(i, j, k)) {
					...
					size_t n = ligand.atoms_tree.radiusSearch(cp, s_searchRad, ret_matches);
					if (n > 0) {
						receptor_interface[i][j][k] = 1;
//						r_negativeDXPotentials[i][j][k] = receptor.surface->negativeDXPotentials[i][j][k];
//						r_positiveDXPotentials[i][j][k] = receptor.surface->positiveDXPotentials[i][j][k];
					}
				}
			}
		}
	...
	ligand_interface = array3D(ligand.length, array2D(ligand.width, array1D(ligand.height, 0)));
//	l_negativeDXPotentials = array3D(ligand.length, array2D(ligand.width, array1D(ligand.height, 0)));
//	l_positiveDXPotentials = array3D(ligand.length, array2D(ligand.width, array1D(ligand.height, 0)));

	for (int i  = 0; i < ligand.length; ++i) {
		for (int j = 0; j < ligand.width; ++j) {
			for (int k = 0; k < ligand.height; ++k) {
				if (ligand.surface->surface->getVoxel(i, j, k)) {
					point3D cp(i, j, k);
					cp /= ligand.surface->resolution;
					cp -= ligand.translation;
					vector<pair<size_t, float> > ret_matches;
					size_t n = receptor.atoms_tree.radiusSearch(cp, s_searchRad, ret_matches);
					if (n > 0) {
						ligand_interface[i][j][k] = 1;
//						l_negativeDXPotentials[i][j][k] = ligand.surface->negativeDXPotentials[i][j][k];
//						l_positiveDXPotentials[i][j][k] = ligand.surface->positiveDXPotentials[i][j][k];
					}
				}
			}
		}
...
void MolecularComplex::outputReceptorInterfacePCDModel(string const & filename) {
	...
	while (!surfaceVoxels.empty()) {
		...
		pDX = r_positiveDXPotentials[x][y][z] - r_negativeDXPotentials[x][y][z];
		if (pDX <= 0) {
			r = 255;

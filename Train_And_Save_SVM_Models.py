# This script trains and saves (pickes) the svm models to be later used for classifying descriptors.
...
def train_svm_model(training_set_path, cat, it, bu, svm_model):
    ...
    with open("%ssvm_model_%s_%s_%s.pkl" % (training_set_path, cat, it, bu), 'wb') as output_svm_model, open("%smin_max_scaler_%s_%s_%s.pkl" % (training_set_path, cat, it, bu), 'wb') as output_min_max_scaler :
        pickle.dump(svm_best, output_svm_model, pickle.HIGHEST_PROTOCOL)
        pickle.dump(min_max_scaler, output_min_max_scaler, pickle.HIGHEST_PROTOCOL)
...
for cat in ["A", "AB", "EI", "ER", "ES", "OG", "OR", "OX"]:
    for it in ["l", "r"] :
        for bu in ["u", "b"] :
            ...
            train_svm_model(training_set_path, cat, it, bu, svm_model)
            print ("*" * 255)